...
public class TeleportG1v1 implements Listener
{
    ...
    public void invClickSPTP(final InventoryClickEvent e) {
        ...
        if (e.isRightClick() || e.isLeftClick()) {
            ...
            if (e.getClickedInventory().contains(Material.GOLD_HELMET)) {
                if (e.getCurrentItem() == null || e.getCurrentItem().getType().equals((Object)Material.AIR)) {
                    return;
                }
                if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase(this.displayNameGame1v1Item)) {
                    final YamlConfiguration spawnPointConfig = YamlConfiguration.loadConfiguration(new File("plugins/LobbySystem/config.yml"));
                    final Object X = spawnPointConfig.get("navigame1v1.X");
                    final Object Y = spawnPointConfig.get("navigame1v1.Y");
                    final Object Z = spawnPointConfig.get("navigame1v1.Z");
