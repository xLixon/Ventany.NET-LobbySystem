        ...
        if (e.isRightClick() || e.isLeftClick()) {
            ...
            if (e.getClickedInventory().contains(Material.DIAMOND_CHESTPLATE)) {
                ...
                if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase(this.displayNameFFAItem)) {
                    ...
                    final Object X = spawnPointConfig.get("naviffa.X");
                    final Object Y = spawnPointConfig.get("naviffa.Y");
                    final Object Z = spawnPointConfig.get("naviffa.Z");
                    final Object world = spawnPointConfig.get("naviffa.WORLD");
                    final Object yaw = spawnPointConfig.get("naviffa.YAW");
                    if (!(X instanceof Double)) {
                        p.sendMessage(String.valueOf(String.valueOf(this.Prefix)) + "§2Location: FFA" + " " + "§rDer Wert \"X\" ist §4KEINE §r Zahl.");
                        return;
                    }
                    if (!(Y instanceof Double)) {
                        p.sendMessage(String.valueOf(String.valueOf(this.Prefix)) + "§2Location: FFA" + " " + "Der Wert \"Y\" ist §4KEINE §r Zahl.");
                        return;
                    }
                    if (!(Z instanceof Double)) {
                        p.sendMessage(String.valueOf(String.valueOf(this.Prefix)) + "§2Location: FFA" + " " + "Der Wert \"Z\" ist §4KEINE §r Zahl.");
                        return;
                    }
                    if (!(world instanceof String)) {
                        p.sendMessage(String.valueOf(String.valueOf(this.Prefix)) + "§2Location: FFA" + " " + "Der Wert \"world\" ist §4KEIN §r Name.");
                        return;
                    }
